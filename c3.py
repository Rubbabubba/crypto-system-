...
try:
    ...
    logger.debug(f"[c3] utils_volatility import skipped: {e}")
try:
    import vol_filter as vf
except Exception as e:
    logger.debug(f"[c3] vol_filter import skipped: {e}")
...
def policy_claim(symbol):
    try: note_trade_event("claim", strategy="c3", symbol=symbol)
    except Exception as e: logger.debug(f"[policy] c3 claim failed: {e}")

def policy_release(symbol):
    try: note_trade_event("release", strategy="c3", symbol=symbol)
    except Exception as e: logger.debug(f"[policy] c3 release failed: {e}")

def scan(*args, **kwargs):
    logger.info("[c3] scan() placeholder called; returning no signals.")
    return {
        "strategy": "c3",
        "ok": True,
        "signals": [],
...
CONFIG = {"enabled": True, "name": "c3", "version": 1}
META = {"id": "c3", "display": "C3", "group": "default"}
