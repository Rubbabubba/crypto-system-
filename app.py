...
@app.post("/scheduler/run")
def scheduler_run(payload: dict = Body(default=None)):
    ...
    tf        = str(payload.get("tf") or os.getenv("SCHED_TIMEFRAME", os.getenv("DEFAULT_TIMEFRAME","5Min") or "5Min"))
    strats    = _csv(payload.get("strats") or os.getenv("SCHED_STRATS", "c1,c2,c3,c4,c5,c6"))
    symbols   = _csv(payload.get("symbols") or os.getenv("SYMBOLS", "BTC/USD,ETH/USD"))
    limit     = int(payload.get("limit") or os.getenv("SCHED_LIMIT", os.getenv("DEFAULT_LIMIT","300") or "300"))
    notional  = float(payload.get("notional") or os.getenv("SCHED_NOTIONAL", os.getenv("DEFAULT_NOTIONAL","25") or "25"))
    dry       = bool(payload.get("dry_run") if payload.get("dry_run") is not None else str(os.getenv("SCHED_DRY","0")).lower() in ("1","true","yes"))

    msg = f"Scheduler pass: strats={','.join(strats)} tf={tf} limit={limit} notional={notional} dry={dry} symbols={','.join(symbols)}"
    log.info(msg)

    # preload bars once per symbol
    bars_cache = {}
    for sym in symbols:
        try:
            bars_cache[sym] = br.get_bars(sym, timeframe=tf, limit=limit)
        except Exception as e:
            log.warning("bars load failed for %s: %s", sym, e)

    def load_scan(mod_name: str):
        m = import_module(mod_name.lower())
        if not hasattr(m, "scan"):
            raise RuntimeError(f"{mod_name}.scan() not found")
        return m.scan
    ...
    for strat in strats:
        try:
            scan = load_scan(strat)
            req = {"timeframe": tf, "limit": limit, "symbols": symbols, "notional": notional}
            ctx = {"timeframe": tf, "limit": limit, "symbols": symbols, "notional": notional, "preloaded_bars": bars_cache}
            intents = scan(req=req, ctx=ctx) or []
            debug_intents.extend(intents)
            for it in intents:
                if it.get("side") in ("buy","sell") and float(it.get("notional", 0)) > 0:
                    actions.append({**it, "strategy": strat})
        except Exception as e:
            log.exception("strategy %s failed: %s", strat, e)

    fills = []
    if not dry:
        for a in actions:
            try:
                res = br.market_notional(a["symbol"], a["side"], float(a["notional"]))
                fills.append({"action": a, "result": res})
                log.info("ORDER %s %s $%.2f -> %s", a["side"], a["symbol"], float(a["notional"]), str(res)[:180])
            except Exception as e:
                log.warning("order failed %s: %s", a, e)

    global _SCHED_LAST
    with _SCHED_LAST_LOCK:
        _SCHED_LAST = {"tf": tf, "strats": ",".join(strats), "symbols": ",".join(symbols),
                       "limit": limit, "notional": notional, "dry_run": dry,
                       "ts": datetime.datetime.utcnow().isoformat() + "Z",
                       "intents": debug_intents[:50]}

    return {"ok": True, "message": msg, "actions": actions, "executed": (not dry), "fills": fills}
