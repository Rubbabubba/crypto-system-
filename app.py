...
APP_VERSION = "2.0.0-hotfix.3-advisorv2blocked"
...
def _log_blocked_event(action_record: Dict[str, Any], source: str = "scheduler_v2") -> None:
    """
    Phase 2 (Learning Engine foundation): persist "blocked / skipped" trade intents.

    We intentionally make this best-effort: failures must NEVER break the scheduler response.
    """
    try:
        ...
        cur.execute(
            ...
            (
                ts,
                action_record.get("strategy"),
                action_record.get("symbol"),
                action_record.get("side"),
                action_record.get("kind"),
                status,
                action_record.get("reason"),
                action_record.get("error") or action_record.get("detail"),
                float(action_record.get("notional") or 0.0),
                1 if action_record.get("dry") else 0,
                source,
            ),
        )
        conn.commit()
...
def advisor_v2_blocked_summary(hours: int = Query(24, ge=1, le=24*30), top_n: int = Query(20, ge=1, le=200)):
    """
    Phase 2: summarize blocked/skipped intents captured by the scheduler.

    Returns counts by reason, symbol, and strategy for the last N hours.
    """
    try:
        import time
        ...
        by_reason = [{"reason": r[0] or "", "count": int(r[1])} for r in rows]
        ...
        by_symbol = [{"symbol": r[0] or "", "count": int(r[1])} for r in rows]
        ...
        by_strategy = [{"strategy": r[0] or "", "count": int(r[1])} for r in rows]
