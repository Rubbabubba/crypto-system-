...
def debug_strategy_scan(payload: Dict[str, Any] = Body(default=None)):
    ...
    # Load all open positions from trades and build a dict keyed by (symbol, strategy)
    pos_list = _load_open_positions_from_trades(use_strategy_col=True)
    positions: Dict[Tuple[str, str], Position] = {}
    for p in pos_list:
        sym = getattr(p, "symbol", "") or ""
        strat_name = getattr(p, "strategy", "") or ""
        if not include_legacy and strat_name.lower() == "legacy":
            continue
        if include_strategy and not strat_name:
            # When include_strategy is True, ignore positions without a strategy tag
            continue
        positions[(sym, strat_name)] = p
...
def debug_positions(include_strategy: bool = True, include_legacy: bool = False):
    ...
    """
    # Load all open positions from trades; we will filter by strategy/legacy below
...
def scheduler_run(payload: Dict[str, Any] = Body(default=None)):
    """
    Backward-compatible wrapper that delegates to ``scheduler_run_v2``.
    It normalizes the incoming payload into the v2 shape and returns the v2 result.
    """
    body = payload or {}

    # Normalize core scheduling parameters
    tf = body.get("tf") or body.get("timeframe") or "5Min"
    limit = int(body.get("limit") or body.get("bars", 300))
    notional = float(body.get("notional") or body.get("size", 40.0))
    symbols = body.get("symbols") or body.get("syms") or body.get("symbol") or "ETH/USD"
    strats = body.get("strats") or body.get("strategies") or body.get("strat") or "c1"
    dry = as_bool(body.get("dry", False))

    cfg = scheduler_run_v2(
        body={
            "tf": tf,
            "limit": limit,
            "notional": notional,
        ...
        dry=dry,
        skip_live=as_bool(body.get("skip_live", False)),
        skip_write=as_bool(body.get("skip_write", False)),
    )
    return cfg
