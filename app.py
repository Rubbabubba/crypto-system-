...
<head>
  ...
  <style>
    body{font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;background:#0b0f14;color:#e7ebef;margin:0}
    .wrap{max-width:1100px;margin:24px auto;padding:0 16px}
    h1{font-weight:600;margin:0 0 16px;font-size:28px}
    .muted{color:#9aa4ad}
    .grid{display:grid;grid-template-columns:1fr;gap:16px}
    @media(min-width:900px){.grid{grid-template-columns:1fr 1fr}}
    .card{background:#121820;border:1px solid #1c2733;border-radius:10px;padding:16px}
    .card h2{margin:0 0 12px;font-size:18px;font-weight:600}
    table{width:100%;border-collapse:collapse}
    th,td{padding:8px;border-bottom:1px solid #1c2733;text-align:left}
    code, pre{font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
    .pill{display:inline-block;padding:2px 8px;border-radius:999px;background:#1c2733;color:#cbd5df;font-size:12px}
    .ok{color:#8ff0a4}
    .bad{color:#ff8383}
  </style>
</head>
<body>
  <div class="wrap">
    <h1>Crypto System <span class="muted">/ Dashboard</span></h1>
    ...
    <div class="grid">
      <div class="card">
        <h2>Tradeable Now</h2>
        <div class="muted" style="margin-bottom:8px">Based on whitelist + time windows + mutex; refreshes every 60s.</div>
        <table id="tradeable">
          <thead><tr><th>Strategy</th><th>Symbol</th><th>Window</th><th>Status</th></tr></thead>
          <tbody></tbody>
        </table>
      </div>
      ...
      <div class="card">
        <h2>P&amp;L Summary</h2>
        <pre id="pnl">// waiting…</pre>
      </div>
    </div>

    <div class="card">
      <h2>Journal / Fills</h2>
      <button id="sync">Sync Fills</button>
      <pre id="journal">// waiting…</pre>
    </div>
...
async function refreshConfig(){
  ...
  $('#cfg').innerHTML = `<div><b>Symbols:</b> ${j.SYMBOLS.join(', ')}<br/><b>Strategies:</b> ${j.STRATEGIES.join(', ')}</div>`;
}

// Tradeable Now
async function refreshTradeable(){
  const r = await fetch('/status/tradeable');
  const j = await r.json();
  const rows = j.allowed.map(x=>{
    const w = x.window ? `${x.window.start}-${x.window.end}` : '—';
    return `<tr><td>${x.strategy}</td><td>${x.symbol}</td><td>${w}</td><td class="ok">ok</td></tr>`;
  }).join('') + (j.blocked.slice(0,10).map(x=>{
    const w = x.window ? `${x.window.start}-${x.window.end}` : '—';
    return `<tr><td>${x.strategy}</td><td>${x.symbol}</td><td>${w}</td><td class="bad">${x.reason}</td></tr>`;
  }).join('') || '');
  $('#tradeable tbody').innerHTML = rows || `<tr><td colspan="4" class="muted">No symbols are eligible right now.</td></tr>`;
}

// PnL
async function refreshPnL(){
  const r = await fetch('/pnl/summary'); const j = await r.json();
  $('#pnl').textContent = fmtJSON(j);
}

// Journal
async function syncJournal(){
  const r = await fetch('/journal/sync', {method:'POST', headers:{'content-type':'application/json'}, body:'{}'});
  const j = await r.json();
  $('#journal').textContent = fmtJSON(j);
}
$('#sync').addEventListener('click', syncJournal);

// boot
refreshConfig(); refreshTradeable(); refreshPnL();
setInterval(refreshTradeable, 60000);
</script>
</body>
</html>
...
# -------------------- API: Tradeable (policy view) --------------------
from typing import Dict, Any
from datetime import datetime
from guard import load_policy, guard_allows

@app.get('/status/tradeable')
def status_tradeable() -> Dict[str, Any]:
    cfg = config()
    syms = cfg['SYMBOLS']
    ...
    blocked = []
    now = datetime.utcnow()
    mutex_state = {}  # not tracking live mutex here; this is a static policy view
    for st in strats:
        for sym in syms:
            ok, reason = guard_allows(policy, sym, st, now, mutex_state, expected_move_pct=None, atr_pct=None)
            if ok:
                # pull declared window if any
                win = (policy.windows or {}).get('windows', {}).get(st, {})
                allowed.append({'strategy': st, 'symbol': sym, 'window': {'start': win.get('hours', [])[0] if win.get('hours') else None, 'end': win.get('hours', [])[-1] if win.get('hours') else None}})
            else:
                win = (policy.windows or {}).get('windows', {}).get(st, {})
                blocked.append({'strategy': st, 'symbol': sym, 'window': {'start': win.get('hours', [])[0] if win.get('hours') else None, 'end': win.get('hours', [])[-1] if win.get('hours') else None}, 'reason': reason})
    return {'ok': True, 'time': datetime.utcnow().isoformat()+"Z", 'allowed': allowed, 'blocked': blocked}
