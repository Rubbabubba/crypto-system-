...
import threading
import time
from symbol_map import KRAKEN_PAIR_MAP, to_kraken, from_kraken
...
def from_kraken_pair_to_app(pair: str) -> str:
    """
    Convert a Kraken pair code (e.g. 'XBTUSD', 'AVAXUSD', 'SOLUSD') to the
    app's UI symbol format 'BTC/USD', 'AVAX/USD', etc.

    Uses symbol_map.from_kraken for known mappings and falls back to a simple
    "BASE/QUOTE" split for anything unknown.
    """
    if not pair:
        return ""
    try:
        # Primary path: use the canonical mapping from symbol_map
        return from_kraken(pair)
    except Exception:
...
# ============================================================================
# PnL v2 helpers — position-aware engine on top of raw `trades` fills
# ============================================================================

def _pnl2__parse_ts_to_epoch(s: Optional[str]) -> Optional[float]:
    """Accept 'YYYY-MM-DD' or ISO string, return epoch seconds (UTC)."""
    if not s:
        return None
    try:
        s2 = s.replace("Z", "").strip()
        ...
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=py_datetime.timezone.utc)
        else:
            dt = dt.astimezone(py_datetime.timezone.utc)
        return dt.timestamp()
    except Exception:
        return None


def _pnl2__load_fills(start: Optional[str], end: Optional[str]) -> List[Fill]:
    """Load raw fills from trades table and adapt to PositionEngine.Fill."""
    start_ts = _pnl2__parse_ts_to_epoch(start)
    end_ts = _pnl2__parse_ts_to_epoch(end)
    ...
    fills: List[Fill] = []
    for txid, ts_, symbol, side, price, volume, fee in rows:
        ...
        try:
            fills.append(
                Fill(
                    ...
                    strategy="misc",  # placeholder, strategies can be layered via symbol mapping later
                    txid=str(txid or ""),
                )
            )
        except Exception:
            # skip malformed rows instead of blowing up
            continue
    return fills


def _pnl2__price_lookup(symbol: str) -> float:
    """Get a best-effort last price for a UI symbol like 'BTC/USD'."""
    try:
        import broker_kraken  # local adapter
        px = broker_kraken.last_price(symbol)
...
# ============================================================================
# PnL v2 API — uses position_engine over raw fills
# ============================================================================

@app.get("/pnl2/summary")
def pnl2_summary(start: Optional[str] = None, end: Optional[str] = None):
    fills = _pnl2__load_fills(start, end)
    eng = PositionEngine()
    eng.process_fills(fills)
    snap = eng.snapshot(price_lookup=_pnl2__price_lookup)
    return {"ok": True, **snap}
...
def pnl2_by_strategy(start: Optional[str] = None, end: Optional[str] = None):
    ...
    for strat, row in snap["per_strategy"].items():
        rows.append({"strategy": strat, **row})
    return {
        "ok": True,
        "table": "trades",
        "start": start,
