...
def _bars_fetch_with_retries(symbol: str, timeframe: str, limit: int = 300):
    ...
    try:
        ...
        raise RuntimeError(f"failed_to_import_br_router:{e}")
...
def scheduler_run_v2(payload: Dict[str, Any] = Body(default=None)):
    ...
    try:
        for it in list(final_intents):
            ...
            # We approximate executability here using current journal qty and last price (no broker calls).
            try:
                if kind_k in ("exit", "take_profit") and (str(getattr(it, "side", "") or "").strip().lower() == "sell"):
                    key_k = (sym_k, str(getattr(it, "strategy", "") or ""))
                    pm_pos_k = positions.get(key_k)
                    qty_k = float(getattr(pm_pos_k, "qty", 0.0) or 0.0) if pm_pos_k is not None else 0.0
                    px_k = float(_last_price_safe(sym_k) or 0.0)
                    notional_k = abs(qty_k) * px_k if px_k > 0 else 0.0
                    exit_min_usd = float(os.getenv("EXIT_MIN_NOTIONAL_USD", "6") or 6)
                    if qty_k <= 0.0 or (notional_k > 0 and notional_k < exit_min_usd):
                        pr = -5  # below entry/scale
                        telemetry.append({
                            "stage": "symbol_gate_nonexec_exit_demoted",
                            "symbol": sym_k,
