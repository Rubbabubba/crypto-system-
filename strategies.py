...
def sig_c5_alt_mom(close, regimes: Regimes):
    # Simple alt-mom variant favoring strong-trend names
...
def sig_c6_rel_to_btc(close, regimes: Regimes, ref_close_btc: Optional[np.ndarray] = None):
    # Relative momentum vs BTC proxy (if provided from contexts); otherwise fallback to momentum
...
# ====== MTF confirm (5m trend confirms 1m entries) ======
def mtf_confirm(action: str, reg5: Regimes) -> bool:
    if action == "buy":
        return reg5.sma_fast > reg5.sma_slow
    if action == "sell":
        return reg5.sma_fast < reg5.sma_slow
    return True

# ====== sizing ======
def size_from_atr(price: float, atr: float, target_risk_usd: float = 10.0, atr_mult: float = 1.0) -> Tuple[float,float]:
...
class StrategyBook:
    ...
    def scan(self, req: ScanRequest, contexts: Dict[str, Optional[Dict[str, Any]]]) -> List[ScanResult]:
        ...
        # For c6 relative, grab BTC close if present
        ref_btc = None
        ...
        # Select top-K by score among actionable
        ...
        for r in results:
            ...
            if (not r.selected) and r.action in ("buy","sell") and r.symbol not in chosen:
                # downgrade reason if cut by score/rank
                r.action = "flat"
