...
Version: 2025.10.11-crypto-v3f

v3f changes:
- Strong broker selection: default to Kraken, auto-detect via KRAKEN_* env.
- Broker-specific trading gating: TRADING_ENABLED also requires KRAKEN_TRADING=1 when BROKER=kraken.
- Loud startup log + new /diag/broker endpoint that reports selected broker/module and flags.
- Keeps full dashboard, strategy hot-reload, universe builder, diagnostics, exit nanny, etc. from v3e.
"""
...
APP_VERSION = os.getenv("APP_VERSION", "2025.10.11-crypto-v3f")
...
# -----------------------------------------------------------------------------
# Broker switch (Alpaca vs Kraken) — with strong defaults for Kraken
# -----------------------------------------------------------------------------
def _select_broker() -> str:
    env_choice = (os.getenv("BROKER") or "").strip().lower()
    if env_choice in ("alpaca", "kraken"):
        return env_choice
    # Auto-detect Kraken if keys/flags are present; otherwise default to Kraken (Texas-safe).
    if os.getenv("KRAKEN_API_KEY") or os.getenv("KRAKEN_TRADING", "0").lower() in ("1", "true"):
        return "kraken"
    return "kraken"

BROKER = _select_broker()
if BROKER == "kraken":
    import broker_kraken as br  # must exist alongside this app
else:
    import broker as br         # Alpaca implementation

# Compute final trading enabled with broker-specific guard
if BROKER == "kraken":
    _broker_live_flag = os.getenv("KRAKEN_TRADING", "0").lower() in ("1", "true")
else:
    # If you ever re-enable Alpaca for stocks, you could gate here by ALPACA_TRADING
    _broker_live_flag = True

TRADING_ENABLED = bool(TRADING_ENABLED_BASE and _broker_live_flag)

BROKER_INFO = {
    "selected": BROKER,
    "module": getattr(br, "__name__", "unknown"),
    "trading_enabled_base": TRADING_ENABLED_BASE,
    "broker_live_flag": _broker_live_flag,
    "trading_enabled_final": TRADING_ENABLED,
}

# Loud message on startup path selection happens in @startup as well
log.info("Broker selected: %s (module=%s) trading_enabled_final=%s", BROKER, BROKER_INFO["module"], TRADING_ENABLED)
...
def _builtin_candidates() -> List[str]:
    # Focus on widely-available USD pairs; extend with CANDIDATE_SYMBOLS env as needed.
    base = [
...
_positions_cache: Dict[str, Dict[str, float]] = {}  # populated by /init/positions
...
async def _scan_bridge(strat: str, req: Dict[str, Any], dry: bool = False) -> List[Dict[str, Any]]:
    ...
    req["symbols"] = syms
    req["dry"] = dry or (not TRADING_ENABLED)  # make sure dry if trading disabled
    req["raw"] = dict(req)
...
async def _scheduler_loop():
    ...
    try:
        while _running:
            ...
            log.info("Scheduler tick complete (dry=%d symbols=%d broker=%s) — sleeping %ss",
                     int(dry_flag), len(_CURRENT_SYMBOLS), BROKER, SCHEDULE_SECONDS)
...
async def _startup():
    ...
    log.info("App startup (version=%s); broker=%s module=%s trading_enabled=%s; interval=%ss; strategies=%s; symbols=%d",
             APP_VERSION, BROKER, BROKER_INFO["module"], TRADING_ENABLED, SCHEDULE_SECONDS,
             ",".join(sorted(ACTIVE_STRATEGIES)), len(_CURRENT_SYMBOLS))
...
@app.get("/diag/broker", response_class=JSONResponse)
def diag_broker():
    # Minimal ping to broker layer to surface failures early:
    try:
        probe = br.get_bars(["BTC/USD"], timeframe=DEFAULT_TIMEFRAME, limit=1) or {}
        ok = True
    except Exception as e:
        probe = {"error": str(e)}
        ok = False
    return {
        "ok": ok,
        "info": BROKER_INFO,
        "probe": {k: (len(v) if isinstance(v, list) else v) for k, v in (probe or {}).items()},
    }
...
async def diag_alpaca():
    # Kept for compatibility; may fail when BROKER=kraken (that's fine).
    try:
...
if __name__ == "__main__":
    ...
    log.info("Launching Uvicorn on 0.0.0.0:%d (version %s, broker=%s, trading=%s)",
